		...
		if ( GetDvarInt("scr_zombie_spawn_in_view") )
		{
			...
			foreach ( loc in level.zm_loc_types[ "zombie_location" ] )
			{
				...
				if ( dot > 0.707 && dist <= GetDvarInt( "scr_zombie_spawn_in_view_dist" ) )
				{
					...
					//spots[spots.size] = loc;
					/# debugstar( loc.origin, 1000, WHITE ); #/
				}
			}
			...
			if ( isdefined( look_loc ) )
			{
				...
				/# debugstar( look_loc.origin, 1000, GREEN ); #/
			}
...
#/
	...
	{
		debugstar( spot.origin, GetDvarInt( "scr_spawner_location_time" ), GREEN );
...
/#
	...
	while( 1 )
	{
		print3d( self.origin +( 0, 0, 64 ), self.zone_spawned_from, WHITE ); 
		WAIT_SERVER_FRAME;
	}
#/
}
