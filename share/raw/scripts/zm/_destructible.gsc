...
#insert scripts\zm\_destructible.gsh;
...
function __init__()
{
	...
	clientfield::register( "scriptmover", DESTRUCTIBLE_CLIENTFIELD, VERSION_SHIP, DESTRUCTIBLE_CLIENTFIELD_NUM_BITS, "int" );
...
function init_explosions()
{
	...
	while( i < 32 )
	{
		...
		ARRAY_ADD( level.explosion_manager.a_explosions, sExplosion );
		i++;
	}
}
...
function physics_explosion_and_rumble( origin, radius, physics_explosion )
{
	...
	assert( radius <= pow( 2, DESTRUCTIBLE_CLIENTFIELD_NUM_BITS ) - 1 );
	
	if( IS_TRUE( physics_explosion ) )
	{
		radius += ( 1 << ( DESTRUCTIBLE_CLIENTFIELD_NUM_BITS - 1 ) );
	}
	
	WAIT_SERVER_FRAME;
	
	sExplosion clientfield::set( DESTRUCTIBLE_CLIENTFIELD, radius );
	...
	if( IsSubStr( destructible_event, "explosive" ) )
	{
		...
		switch ( damage_type )
		{
			case "concussive":
				...
				{
					explosion_radius = CONCUSSIVE_DAMAGE_RADIUS_LG;
				}
				else
				{
					explosion_radius = CONCUSSIVE_DAMAGE_RADIUS_SM;
				}
			...
			case "electrical":
				...
				if(explosion_radius_type == "large")
				{
					explosion_radius = ELECTRICAL_DAMAGE_RADIUS_LG;
				}
				else
				{
					explosion_radius = ELECTRICAL_DAMAGE_RADIUS_SM;
				}
			break;
			...
			case "incendiary":
				if(explosion_radius_type == "large")
				{
					explosion_radius = INCENDIARY_DAMAGE_RADIUS_LG;
				}
				else
				{
					explosion_radius = INCENDIARY_DAMAGE_RADIUS_SM;
				}
			break;
		}
	}
...
function explosive_incendiary_explosion( attacker, explosion_radius, physics_explosion )
{
	if ( !IsVehicle( self ) )
	{
		...
		if ( isdefined( attacker ))
		{
			self RadiusDamage( self.origin + offset, explosion_radius, INCENDIARY_EXPLOSION_DMG_MAX, INCENDIARY_EXPLOSION_DMG_MIN, attacker, "MOD_BURNED", GetWeapon( "incendiary_fire" ) );
		}
		else
		{
			self RadiusDamage( self.origin + offset, explosion_radius, INCENDIARY_EXPLOSION_DMG_MAX, INCENDIARY_EXPLOSION_DMG_MIN );
		}
		physics_explosion_and_rumble(self.origin, 255, physics_explosion );
...
function explosive_electrical_explosion( attacker, explosion_radius, physics_explosion )
{
	if ( !IsVehicle( self ) )
	{
		...
		if ( isdefined( attacker ))
		{
			self RadiusDamage( self.origin + offset, explosion_radius, ELECTRICAL_EXPLOSION_DMG_MAX, ELECTRICAL_EXPLOSION_DMG_MIN, attacker, "MOD_ELECTROCUTED" );
		}
		else
		{
			self RadiusDamage( self.origin + offset, explosion_radius, ELECTRICAL_EXPLOSION_DMG_MAX, ELECTRICAL_EXPLOSION_DMG_MIN );
		}
		physics_explosion_and_rumble(self.origin, 255, physics_explosion );
...
function explosive_concussive_explosion( attacker, explosion_radius, physics_explosion )
{
	if ( !IsVehicle( self ) )
	{
		...
		if ( isdefined( attacker ))
		{
			self RadiusDamage( self.origin + offset, explosion_radius, CONCUSSIVE_EXPLOSION_DMG_MAX, CONCUSSIVE_EXPLOSION_DMG_MIN, attacker, "MOD_GRENADE" );
		}
		else
		{
			self RadiusDamage( self.origin + offset, explosion_radius, CONCUSSIVE_EXPLOSION_DMG_MAX, CONCUSSIVE_EXPLOSION_DMG_MIN );
		}
		physics_explosion_and_rumble(self.origin, 255, physics_explosion );
	}
