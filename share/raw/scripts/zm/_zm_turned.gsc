...
function init()
{
	...
	if ( dvar == "zcleansed" )
	{
		...
		visionset_mgr::register_info( "visionset", "zombie_turned", VERSION_SHIP, level.vsmgr_prio_visionset_zombie_turned, 1, true); //, &vsmgr_lerp_power_up_down, false );
		clientfield::register( "toplayer", "turned_ir", VERSION_SHIP, 1, "int" ); 
	 	clientfield::register( "allplayers", "player_has_eyes", VERSION_SHIP, 1, "int" );		
	 	clientfield::register( "allplayers", "player_eyes_special", VERSION_SHIP, 1, "int" );		
...
function turn_to_zombie()
{
	...
	if (IS_TRUE(self.turned_visionset)) // the turned visionset gets stomped by the laststand visionset if you die as a zombie - deactivate it and reactivate it to force it back on
	{
